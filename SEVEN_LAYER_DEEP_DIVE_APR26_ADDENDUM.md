...
**SDK now exposes 23 signals total via `getHumanConfidence()`.** Three signals weight 0.00 are diagnostic-only.

---

### Env-gate: BotD-only → 6-vector multi-detector
...
1. **Submovement-count v2: count discrete movement events, not individual samples.** Group consecutive-motion samples into one "event" and count events; bypasses the 60Hz Gaussian-noise false-positive.
...
6. **AudioContext baseLatency consistency** — stealth plugins commonly miss spoofing AudioContext latencies to match claimed UA platform.
7. **Microsaccade v2: idle-window definition that excludes mousemove-during-bezier samples.** Either segment by movement events or use only "between-movement" sample pairs (long inter-sample dt > 200ms but small distance).
...
- 23 signals, 3 of which are diagnostic-only (weight 0) but documented honestly
- 6-vector env-gate
...
2. Update Layer 1 (env-gate): BotD-only → 6-vector
