...
- **Empirical validation**: ran the DMTG-class bot (puppeteer-extra-stealth + Bezier + 1/f-per-channel) against the new 7-vector env-gate locally. Result: **the audio probe fired with `reasons: ["no_audioWorklet_getter"]`**. Puppeteer-extra-stealth patches the `AudioContext` constructor surface and the `baseLatency` and `AudioBuffer.sampleRate` native getters — but it does NOT spoof the `AudioContext.prototype.audioWorklet` getter, leaving it as a non-native function. The new probe catches this. Stealth suspicion against this adversary lifted from ~0.10 (chrome.runtime missing only) to **0.30** (chrome.runtime 0.10 + audio 0.20). Still under the 0.50 threshold for `bot=true` on real hardware, but the gap is closing — and on cloud-VM hardware (where WebGL/WebGPU cloud-VM signature would also fire at +0.45), the audio probe pushes the bot decisively over the threshold. Behavioral composite measured 0.502 (within the published 0.55 ± 0.02 band — the new vector is environmental, doesn't move the behavioral score, exactly as designed).
