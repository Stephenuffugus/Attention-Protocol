...
(function(window, document) {
  ...
  //
  // Round-2 R2-NEW-10 fix: Unicode NFC normalization on every string.
  // Pre-fix: "naïve" typed via NFC (composed é) vs NFD (e + combining
  // acute) produced different UTF-8 bytes → different hashes → two
  // valid receipts for the same logical content. Now: every string
  // is NFC-normalized before serialization, matching RFC 8785 §3.4.
  // Object keys are also normalized for the same reason.
  // Browsers + Node 12+ ship String.prototype.normalize natively.
  function _nfc(s) {
  ...
  function _canonicalJSON(obj) {
    ...
    if (typeof obj === 'object') {
      // Build (nfcKey, originalValue) pairs so the value lookup uses
      // the ORIGINAL key (which may be in NFD form on this object)
      // while the canonical output uses the NFC-normalized key.
      var entries = [];
  ...
  function _jsSha256(str) {
    // Round-4 fan-out propagation of R2-NEW-9 UTF-8 SHA-256 fix. The
    // original `if (j >> 8) return ''` path silently emitted empty
    // hashes for any non-ASCII codepoint; round 2 fixed it in
    // attention-receipts.js, round 3 fixed it in proof/sdk, round 4
    // (this commit) fans out to the remaining 3 files including this
    // one. Surrogate-pair-safe legacy fallback for environments
    // without TextEncoder.
    var bytes;
