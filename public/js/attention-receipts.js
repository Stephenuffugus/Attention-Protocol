...
(function(window) {
  ...
  function generateReceipt(params) {
    ...
    // Generate receipt hash synchronously so callers always get a real hash
    receipt.proof.receipt_hash = _sha256Pure(receiptPayload);

    // Also try async Web Crypto for consistency verification (non-blocking)
  ...
  // Round-4 fan-out propagation of R2-NEW-9 UTF-8 fix to public/js
  // (Hostinger production path for stevieweedseed.com). Surrogate-pair-
  // safe legacy fallback for environments without TextEncoder.
